...
def server(input, output, session):
    ...
    def resumen_tipo():
        ...
        mayor = max(VALUE_COLUMNS, key=lambda x: df_rango[x].sum())
        menor = min(VALUE_COLUMNS, key=lambda x: df_rango[x].sum())
        
        total_general = sum(df_rango[col].sum() for col in VALUE_COLUMNS)
    ...
    def respuesta_mas_2023():
        tot = totales_por_vehiculo_anual(2023, VALUE_COLUMNS)
